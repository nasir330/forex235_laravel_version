<x-header />

<body>
    <x-octa-menu />
    <!-- marquee start -->
    <div class="row">
        <div class="marqueeSection col-md-12">
            <marquee direction="left">
                We are working hard to meet our deadlines. Our team is dedicated and motivated to achieve our goals. We
                have a clear plan and are making progress every day. Our efforts are paying off, and we are seeing
                positive results. We will continue to work diligently to ensure success.
            </marquee>
        </div>
    </div>
    <!-- marquee end -->

    <!-- CFD's market start  -->
    <section>
        <div class="row p-4">
            <div class="col-md-8 border text-center p-4">
                <h2 class="border-bottom">Trading CFD's</h1>
                    <p class="forex-market">
                        Trading CFDs with <strong>Forex 235</strong> can be a lucrative investment strategy due to its
                        ability to
                        speculate on various financial instruments without owning the underlying assets. <strong>Forex
                            235</strong>, a
                        popular online broker, offers a wide range of CFDs like currencies, commodities, stocks, and
                        indices. Leverage is a key advantage, allowing traders to control larger positions with less
                        capital, but it also magnifies both profits and losses, necessitating a solid risk management
                        strategy. <strong>Forex 235</strong> provides advanced trading tools such as MetaTrader 4,
                        offering real-time
                        market data, charting capabilities, and customizable indicators. With these tools, traders can
                        analyze market trends, identify entry and exit points, and execute precise trades. This form of
                        trading allows investors to diversify their portfolios and potentially generate profits in both
                        rising and falling markets. It is important, however, to fully understand the risks involved and
                        develop a comprehensive trading plan before engaging in CFD trading with <strong>Forex
                            235</strong>.
                    </p>
            </div>
            <div class="col-md-4 border text-center p-4">
                <img class="img-fluid" src="Assets/images/trading-cfd.webp" alt="Trading on CFD">
            </div>
        </div>
    </section>
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12 paymentFeesSection">
                <div class="row">
                    <div class="col-md-12 text-center bg-dark text-light m-1">
                        <h1 class="text-uppercase">CFD's major pair's list table</h1>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-12">
                        <table class="table table-dark table-striped table-bordered text-center">
                            <thead>
                                <tr>
                                    <th>Instrument/Symbol</th>
                                    <th>Description</th>
                                    <th>Opening Time</th>
                                    <th>Closing Time</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    <td>AUS200</td>
                                    <td>S&P/ASX Index of Australian listed shares</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>CHN.50</td>
                                    <td>Singapore Exchange FTSE China A50 Index</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>COPPER</td>
                                    <td>Copper</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>FRA.40</td>
                                    <td>CAC 40 index of French listed shares</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>GER.30</td>
                                    <td>Dax index of German listed shares</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>JPN225</td>
                                    <td>Nikkei - Japanese Stock Index</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>NAS100</td>
                                    <td>NASDAQ 100 index of US listed shares</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>SPX500</td>
                                    <td>S&P 500 index of US listed Shares</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>UK.100</td>
                                    <td>Financial Times & London - British FTSE100 Index</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>UK.OIL</td>
                                    <td>Brent Crude Oil</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>USA.30</td>
                                    <td>Index of top 30 shares on Wall Street</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>USD.X</td>
                                    <td>USD Index</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>

                                <tr>
                                    <td>#AAPL</td>
                                    <td>Apple Inc.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#AMD</td>
                                    <td>Advanced Micro Devices Inc.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#AMZN</td>
                                    <td>Amazon.com Inc.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#AXP</td>
                                    <td>American Express</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#AZN</td>
                                    <td>AstraZeneca</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#BA</td>
                                    <td>Boeing Co.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#BABA</td>
                                    <td>Alibaba Group Holding Ltd</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#BAC</td>
                                    <td>Bank of America</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#BIDU</td>
                                    <td>Baidu.com Inc.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#CAT</td>
                                    <td>Caterpillar Inc.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#DIS</td>
                                    <td>The Walt Disney Company</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#GE</td>
                                    <td>General Electric Co.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#GLW</td>
                                    <td>Corning Inc.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#GOOGL</td>
                                    <td>Alphabet Inc. - A Shares</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#GS</td>
                                    <td>Goldman Sachs Group Inc.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#HD</td>
                                    <td>Home Depot Inc.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#IBM</td>
                                    <td>International Business Machines Co.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#JNJ</td>
                                    <td>Johnson & Johnson</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#JPM</td>
                                    <td>JPMorgan Chase & Co.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#KO</td>
                                    <td>The Coca-Cola Co.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#LMT</td>
                                    <td>Lockheed Martin Corp.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#MA</td>
                                    <td>Mastercard Inc.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#META</td>
                                    <td>Meta Platforms Inc.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#MMM</td>
                                    <td>3M Co.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#MRK</td>
                                    <td>Merck</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#MRNA</td>
                                    <td>Moderna Inc.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#MS</td>
                                    <td>Morgan Stanley</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#MSFT</td>
                                    <td>Microsoft Corp.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#NFLX</td>
                                    <td>Netflix Inc.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#NKE</td>
                                    <td>Nike Inc.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#PFE</td>
                                    <td>Pfizer</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#SHOP</td>
                                    <td>Shopify Inc.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#TSLA</td>
                                    <td>Tesla Motors Inc.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#TWTR</td>
                                    <td>Twitter Inc.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#UNH</td>
                                    <td>UnitedHealth Group Inc.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#V</td>
                                    <td>Visa Inc.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#VZ</td>
                                    <td>Verizon Communications</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#WBA</td>
                                    <td>Walgreens Boots Alliance Inc.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#WMT</td>
                                    <td>Walmart Inc.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#XOM</td>
                                    <td>Exxon Mobil Corporation</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                <tr>
                                    <td>#ZM</td>
                                    <td>Zoom Video Communications Inc.</td>
                                    <td>Monday | 16:35</td>
                                    <td>Friday | 22:55</td>
                                </tr>
                                

                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- CFD's market  end -->

    <!-- mid footer start -->
    @include('templates.midfooter')
    <!-- mid footer end -->

    <!-- footer start -->
    @include('templates.pagefooter')
    <!-- footer end -->

    <!-- modal start -->
    <section>
        @include('templates.modal.loginmodal')
        @include('templates.modal.registermodal')
    </section>
    <!-- modal end -->

    <x-footer />
